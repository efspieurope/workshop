...
path0 <- "C:/rufibach/70_Service/EFSPI/workshop/20240911_9th_EFSPI_RegStat_Basel/30_registrations/"

path.data     <- paste(path0, "data/", sep = "")
path.code     <- paste(path0, "code/", sep = "")
...
source(paste(path0, "p00_initialize.r", sep = ""))
source(paste(path0, "p05_input_data.r", sep = ""), echo = FALSE)
source(paste(path0, "p07_prepare_data.r", sep = ""), echo = FALSE)
source(paste(path0, "p10_mapping.r", sep = ""), echo = FALSE)
source(paste(path0, "p20_changes_in_registration.r", sep = ""), echo = FALSE)
...
t2 <- prepTable2(t2 = t2, cut = cut)

par(las = 1, mar = c(4.5, 4.5, 3, 1))
ylim <- c(0, 1.1 * max(t2))
xx <- barplot(t2, ylab = "number of registrations", main = paste("Total number of registrations: ", dim(reg)[1], "\n Reason for registrations fee waived", sep = ""), col = 2, ylim = ylim)
text(x = xx, y = t2, label = paste(t2, " (", round(100 * t2 / sum(t2)), "%)", sep = ""), pos = 3, cex = 0.8, col = 1)
```
