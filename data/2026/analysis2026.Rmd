---
title: "EFSPI regulatory statistics workshop 2026: analysis of registrations"
...
source(paste("C:/rufibach/70_Service/EFSPI/workshop/20260819_11th_EFSPI_RegStat_Basel/30_registrations/p00_initialize.r", sep = ""))
...
# Registrations {.tabset .tabset-fade .tabset-pills}
...
par(las = 1, mar = c(4.5, 4.5, 3, 1), cex = 0.85)
ylim <- c(0, 1.1 * max(t2))
...
par(las = 2, mar = c(21, 4.5, 3, 1))
ylim <- c(0, 1.4 * max(t2))
xx <- barplot(t2, ylab = "number of registrations", main = paste("Total number of registrations: ", sum(t2), "\n institution", sep = ""), col = 4, ylim = ylim)
text(x = xx, y = t2, label = paste(t2, " (", round(100 * t2 / sum(t2)), "%)", sep = ""), pos = 3, offset = 2, cex = 0.8, col = 1, srt = 90)
```
...
t2 <- data.frame("ha" = ha, "freq" = as.vector(dat[, 1] + dat[, 2]), "face-to-face" = dat[, "Face-to-face (300 Euro)"], "virtual" = dat[, "Virtual (100 Euro)"])
t2 <- t2[order(t2$freq), ]
...
par(las = 1, mar = c(4.5, 4.5, 3, 1), cex = 0.9)
ylim <- c(0, 1.1 * max(t2))
...
# Analysis of block booking {.tabset .tabset-fade .tabset-pills}
...
kable(t(cbind("Company" = f2fblock, "Pre-booked F2F seats" = f2fblock_n)))
```
...
# Analysis of face-to-face attendance and catering {.tabset .tabset-fade .tabset-pills}
...
b <- sum(f2fblock_n) - sum(pmin(table(filter(reg2, inst2 %in% f2fblock & mode == "Face-to-face (300 Euro)")$inst2), min(f2fblock_n)))

# contributors who did not register yet
c <- nrow(filter(contnotreg, is.na(noshow_sc)))
...
## Meals and wine tasting for those registered for face-to-face attendance

```{r, echo = FALSE, fig.cap = "", fig.align = "center", fig.width = 7, fig.height = 10}
t1 <- filter(reg2[, all], reg2$mode == "Face-to-face (300 Euro)")
t2 <- apply(t1, 2, sum)
par(las = 2, mar = c(20, 4.5, 3, 1))
ylim <- c(0, 1.2 * max(t2))
xx <- barplot(t2, ylab = "number of registrations", main = paste("Total number of face-to-face registrations: ", nrow(t1), "\n meals and wine tasting", sep = ""), col = 2, ylim = ylim)
text(x = xx, y = t2, label = paste(t2, " (", round(100 * t2 / sum(t2)), "%)", sep = ""), pos = 3, cex = 0.8, col = 1)
```


## Meals and wine tasting for those registered for face-to-face attendance, Novartis and Roche only

```{r, echo = FALSE, fig.cap = "", fig.align = "center", fig.width = 7, fig.height = 10}
t1 <- filter(reg2[, all], reg2$inst2 %in% c("Roche") & reg2$mode == "Face-to-face (300 Euro)")
t2 <- apply(t1, 2, sum)
par(las = 2, mar = c(20, 4.5, 3, 1))
ylim <- c(0, 1.2 * max(t2))
xx <- barplot(t2, ylab = "number of registrations", main = paste("Total number of face-to-face registrations: ", nrow(t1), "\n meals and wine tasting", sep = ""), col = 2, ylim = ylim)
...
## Meals and wine tasting for those registered for face-to-face attendance, all except Novartis and Roche

```{r, echo = FALSE, fig.cap = "", fig.align = "center", fig.width = 7, fig.height = 10}
t1 <- filter(reg2, reg2$mode == "Face-to-face (300 Euro)")
t1 <- filter(t1, (t1$inst2 %in% c("Roche", "Novartis") == FALSE))
t1 <- t1[, all]
t2 <- apply(t1, 2, sum)
par(las = 2, mar = c(20, 4.5, 3, 1))
ylim <- c(0, 1.2 * max(t2))
...
Analysis will be provided after the event.
...
tab <- tableby(inst2 ~ noshow, total = TRUE, test = FALSE,
                data = filter(reg2, (inst %in% sel) & (mode == "Face-to-face (300 Euro)")))
summary(tab)
```
