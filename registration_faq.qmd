...
waived_who <- as.data.frame(read_excel(paste(path, "/data/waived_who.xlsx", sep = ""), col_types = NULL))
...
`r res["fee_f"]` Euro for face-to-face and `r res["fee_v"]` Euro for virtual attendance. To register please pay `r res["fee_f"]` / `r res["fee_v"]` Euro (depending on whether you attend in-person or virtually) to EFSPI as contribution to the `r res["issue"]`.

Swift/BIC: SYBKDK22<br>
IBAN: DK4170450001632890<br>
EFSPI c/o Thor Schütt Svane Nielsen<br>
Peberlyk 4<br>
DK-6200 Aabenraa<br>
Danmark<br>
Reference: Regulatory 26<br>
...
waived_who[is.na(waived_who)] <- ""
kable(waived_who[waived_who$waived == 1, "who"], align = "ll", row.names = FALSE, col.names = "")
```

When registering simply chose the virtual or face-to-face option in `r res["reg_form"]` but do not pay the registration fee.

The registration fee is **NOT** waived for


```{r, include=TRUE, echo=FALSE}
waived_who[is.na(waived_who)] <- ""
kable(waived_who[waived_who$waived == 0, "who"], align = "ll", row.names = FALSE, col.names = "")
```
...
Some time after registration (it may take 1-2 weeks) you will receive calendar invites for the two days. Please watch out for these invites, this is your confirmation of registration. If you did not receive them there is a high probability you entered your email address with a typo. If that is the case please re-register with the correct email address.

# Will the event be recorded?

Yes. Recordings will be posted [here](data/2026/slides.html), `r colFmt("pending speaker approval")`.

# Will slidedecks be shared after the event?

Yes. Slidedecks will be posted [here](data/2026/slides.html), `r colFmt("pending speaker approval")`.
...
- Block booking for face-to-face attendance:
  - 8500 Euro up to 35 participants per company, i.e. you reach break even with this option if at least 29 colleagues from your company register for face-to-face attendance to the workshop.
  - If your company intends to register more than 35 participants for face-to-face attendance please reach out to `r res["chairmail"]`.
...
- Block booking for virtual attendance: 3000 Euro for an unlimited number of participants per company, i.e. you reach break-even with this option if at least 30 colleagues from your company register for virtual attendance to the workshop.
...
For `r res["year"]`, `r res["block_virtual"]` have a a block booking for virtual participation, i.e. all colleagues working at these companies do not need to pay a registration fee for virtual participation. Face-to-face registration fee remains the same.
...
# I have registered for face-to-face attendance but have changed my mind and cannot attend, not even virtually. Will my registration fee be reimbursed?
