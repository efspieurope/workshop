...
For contributing academics (incl. students), members of the Scientific Program Committee, session chairs, presenters, poster presenters, and invited panelists we are able to offer the following benefits, depending on your affiliation:

```{r, include=TRUE, echo=FALSE}
waived[is.na(waived)] <- ""
kable(waived, align = "llccccc")
```
However, once registration is open we ask you to still register.
...
We would be very grateful to receive a stable draft of your presentation slidedeck by `r colFmt("4th September 2022", "red")`. This, in order to be flexible and to potentially run the slides from our end in case of issues with the internet connection.
