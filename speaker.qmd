...
You have agreed to contribute to the `r res["program"]`. Many thanks for that!
...
However, if the registration fee is waived for you please still register using `r inputProg(regtext = "this form")["registration"]`.
...
Please send all these materials to [`r res["chair"]`](mailto:`r res["chairmail"]`) and cc: your respective session chairs.
...
- Remind speakers to send slides to [`r res["chair"]`](mailto:`r res["chairmail"]`) by `r colFmt(res["deadline_slides"])` with cc: the session chairs.
